            ...
            case 'GET':
                ...
                case 'POST':
                    ...
                    if (validStatus == '') {
                        ...
                        console.log("Post0000000000000000000");
                        res.writeHead(400);
                        res.end(`Error: ${validStatus}`);
                    }
                    break;
                case 'PUT':
                    let modifiedContact = await getPayload(req);
                    validStatus = validateBookmark(modifiedContact);
                    if (validStatus == '') {
                        if (!isNaN(id)) {
                            if (!('Id' in modifiedContact)) modifiedContact.Id = id;
                            if (modifiedContact.Id == id) {
                                ...
                                if (storedContact != null) {
                                    storedContact.Title = modifiedContact.Title;
                                    storedContact.Url = modifiedContact.Url;
                                    storedContact.Category = modifiedContact.Category;
                                    fs.writeFileSync(bookmarksFilePath, JSON.stringify(bookmarks));
                                    res.writeHead(200);
                                    res.end();
                                } else {
